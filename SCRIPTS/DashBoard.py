...
from EXP7 import Exp7_class
from EXP8 import Exp8_class
import sys
import os
import sheets
remark = []
...
class VlabWindow(QMainWindow):    
    ...
    def DefPage(self):
        ...
        self.but7 = EXP_BUTTON(" Experiment 7 ", self.stacked_layout, "Ph_MetriTitration", Exp7_class(self.stacked_layout), self)
        self.but8 = EXP_BUTTON(" Experiment 8 ", self.stacked_layout, "Qualitative_Analysis", Exp8_class(self.stacked_layout), self)
        ...
        self.defLab7 = QLabel("Ph Metri titration")
        self.defLab8 = QLabel("IDENTIFICATION OF SOME ORGANIC COMPOUNDS")

        self.defLay = QGridLayout()

        for i, j,k in zip([self.but1, self.but2, self.but3, self.but4, self.but5, self.but6, self.but7, self.but8],
                        [self.defLab, self.defLab2, self.defLab3, self.defLab4, self.defLab5, self.defLab6,self.defLab7,self.defLab8],
                        [1, 2, 3, 4,5,6,7,8]):
        ...
        self.cb.addTab(self.tab7,"Exp 7")
        self.cb.addTab(self.tab8,"Exp 8")
        self.createSUB()
    ...
    def createSUB(self):
        ...
        strength_naoh_7 = QLabel("Strength of NaOh(N):")
        self.strength_naoh_7 = QLineEdit()

        eq_pt = QLabel("Equivalence Point")
        self.eq_pt = QLineEdit()

        ac_pka = QLabel("PKaof acetic acid")
        self.ac_pka = QLineEdit()


        self.Submitexp7 = QPushButton("SUBMIT")
        self.Submitexp7.clicked.connect(self.SUBMIT7)
        gridLay7 = QGridLayout()
        flag = 0
        for i,j in zip([strength_naoh_7, eq_pt, ac_pka], [self.strength_naoh_7, self.eq_pt, self.ac_pka]):
            gridLay7.addWidget(i, flag, 0, 1, 3)
            gridLay7.addWidget(j, flag, 3, 1, 1)
            flag = flag+1
        gridLay7.addWidget(self.Submitexp7, flag, 3, 1, 1)
        self.tab7.setLayout(gridLay7)
    ...
    def SUBMIT7(self):
        ...
        if(self.but7.exp1Inst):
            dataGEN = self.but7.expWid.getUnknowns()
            for i in dataGEN:
                print(i)
            '''labels = ["Strength of naOh(N)", "equivalence point(ML)", "Acetic acid pKa"]
        
            dataIN= [self.strength_naoh_7.text(), self.eq_pt.text(),
                       self.ac_pka.text()]
            
            DATA_Misc = ["exp7", remark[1], str(datetime.now()), 7, k]
            
            self.SUBMIT(7, DATA_Misc, labels, dataIN, dataGEN)
        else:
            self.notPerform()'''
